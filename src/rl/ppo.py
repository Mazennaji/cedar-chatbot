...
class PPOTrainer:
    ...
    def train_reward_model(
        ...
        metrics: dict[str, list[float]] = {"losses": [], "avg_reward": []}
        reward_model.train()
        ...
        for epoch in range(epochs):
            ...
            for batch in loader:
                ...
                preferred  = scores[sorted_idx[0::2]]
                rejected   = scores[sorted_idx[1::2]]
                min_len    = min(len(preferred), len(rejected))
                if min_len == 0:
                    continue

                loss = criterion(preferred[:min_len], rejected[:min_len])
            ...
            logger.info(
                "Epoch %d/%d — loss: %.4f, avg reward: %.4f",
                epoch + 1, epochs, avg_loss, avg_reward,
            )
